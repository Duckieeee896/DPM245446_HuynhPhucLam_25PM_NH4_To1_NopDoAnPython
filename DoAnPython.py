...
def center_window(win, w=950, h=600): # Tăng chiều cao một chút
...
def load_data():
    ...
    try:
        cur = conn.cursor()
        # Đảm bảo thứ tự cột trong DB khớp với thứ tự hiển thị
...
def clear_input():
    ...
    cbb_mon.set("")
    cbb_tobomon.delete(0, tk.END) # Xóa nhập liệu tổ bộ môn
    entry_ma.focus()
...
def ThemGiaoVien():
    ...
    monday = cbb_mon.get()
    tobomon = cbb_tobomon.get() # Lấy dữ liệu tổ bộ môn
...
def XoaGiaoVien():
    ...
    confirm = messagebox.askyesno("Xác nhận", f"Bạn có chắc muốn xóa giáo viên có mã {ma}?")
    if confirm:
        conn = connect_db()
        try:
            cur = conn.cursor()
...
def LuuGiaoVien():
    ...
    try:
        ...
        sql = """UPDATE giaovien
                 ...
                 WHERE ma_gv = %s"""
        val = (holot, ten, phai, ngaysinh, monday, tobomon, ma)
        cur.execute(sql, val)
        conn.commit()
...
def TimKiem():
    ...
    center_window(search_win, 350, 150)

    tk.Label(search_win, text="Nhập từ khóa (Mã, Tên, Tổ bộ môn):").pack(pady=10)
    entry_search = tk.Entry(search_win, width=35)
    ...
    tk.Button(search_win, text="Tìm kiếm", command=ThucHienTim, bg="#3498db", fg="white").pack(pady=10)
...
def XuatExcel():
    ...
    try:
        ...
        if file_path:
            cur = conn.cursor()
            cur.execute("SELECT ma_gv, ho_lot, ten, phai, ngay_sinh, mon_day, tobomon FROM giaovien")
            rows = cur.fetchall()
            
            wb = Workbook()
            ws = wb.active
            ws.title = "Danh Sách Giáo Viên"
            headers = ["Mã GV", "Họ Lót", "Tên", "Giới Tính", "Ngày Sinh", "Môn Dạy", "Tổ Bộ Môn"]
            ws.append(headers)
            
            header_font = Font(bold=True, color="FFFFFF")
            fill_style = PatternFill(start_color="2c3e50", end_color="2c3e50", fill_type="solid")
...
lbl_title = tk.Label(root, text="QUẢN LÝ GIÁO VIÊN TRƯỜNG PHỔ THÔNG", font=("Arial", 20, "bold"), fg="#d35400")
...
cbb_mon = ttk.Combobox(frame_info, values=["Toán", "Vật Lý", "Hóa Học", "Sinh Học", "Ngữ Văn", "Tiếng Anh", "Lịch Sử", "Địa Lý", "Tin Học", "GDCD", "Thể Dục", "Tin học", "Giáo dục Quốc Phòng"],
...
date_entry = DateEntry(frame_info, width=17, background="darkblue", 
...
tk.Button(frame_btn, text="Tìm Kiếm", width=btn_width, command=TimKiem, bg="#9b59b6", fg="white").grid(row=0, column=5, padx=5)
